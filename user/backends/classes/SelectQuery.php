<?php
	class SelectQuery extends DbConnect
	{
		...
		public function check_if_group_exists ($name,$school_id) {
			$sql = "SELECT 1 
					FROM group_list
					WHERE name = :name
		...
		public function check_if_school_exists ($name,$email) {
			$sql = "SELECT 1 
					FROM school_list
					WHERE name = :name
					AND email = :email
		...
		public function get_group_invite_token ($group_id,$description) {
			$sql = "SELECT invite_token 
					...
					AND status = 'enabled'";
			$check_query = PDO::prepare($sql);
			$check_query->execute([':group_id'=>$group_id,':description'=>$description]);
			// print_r($check_query->errorInfo());
			return $check_query->fetch(PDO::FETCH_ASSOC)['invite_token']; 
		}
