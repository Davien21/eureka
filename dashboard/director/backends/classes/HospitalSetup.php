<?php
	...
	class HospitalSetup extends DBConnect
	{
		...
		public function __toString () {
			return "Name: ".__CLASS__."<br>This Object allows you to handle hospital setup";
		}
		...
		public function add_hospital($director_id,$name,$address,$email,$phone,$invite_key) {
			$sql = "INSERT INTO {$this->table}(director_id,name,address,email,phone,invite_key)
					VALUES(:director_id,:name,:address,:email,:phone,:invite_key)";
			$insert_query = PDO::prepare($sql);
			$insert_query -> 
				execute([
					'director_id'=>$director_id,'name'=>$name,'address'=>$address,
					'email'=>$email,'phone'=> $phone,'invite_key'=>$invite_key
				]);
